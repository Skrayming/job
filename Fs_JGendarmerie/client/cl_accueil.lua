...
function OpenMenuAccueilgendarmerie() 
    if open then 
		open = false
        ...
        RageUI.Button("Appel un Agent a l'accueil", nil, {RightLabel = "→"}, not codesCooldown5 , {
            onSelected = function()
            codesCooldown5 = true 
        TriggerServerEvent('Appel:gendarmerie')
        -- ESX.ShowNotification('~g~Votre message a bien été envoyé aux gendarme.')
        exports['okokNotify']:Alert("Gendarmerie Nationale", "Votre message a bien été envoyé aux gendarmes", 5000, 'success')
            ...
            RageUI.Button("Valider la Demande", "~o~Merci de respecter les Agents.", { Color = {BackgroundColor = { 76, 175, 80, 50}} }, true, {
                onSelected = function()
                    if (nomprenom == nil or nomprenom == '') then
                        ...
                        exports['okokNotify']:Alert("Gendarmerie Nationale", "Vous n\'a pas rempli ton Nom/Prénom", 5000, 'info')
                    elseif (numero == nil or numero == '') then
                        -- ESX.ShowNotification('~r~Vous n\'a pas rempli ton Numéro')
                        exports['okokNotify']:Alert("Gendarmerie Nationale", "Vous n\'a pas rempli ton Numéro", 5000, 'info')
                    elseif (heurerdv == nil or heurerdv == '') then
                        -- ESX.ShowNotification('~r~Vous n\'a pas rempli l\'heure de Rendez-vous')
                        exports['okokNotify']:Alert("Gendarmerie Nationale", "Vous n\'a pas rempli l\'heure de Rendez-vous", 5000, 'info')
                    elseif (rdvmotif == nil or rdvmotif == '' or rdvmotif == "Motif") then
                        -- ESX.ShowNotification('~r~Vous n\'a pas rempli le motif du rendez-vous')
                        exports['okokNotify']:Alert("Gendarmerie Nationale", "Vous n\'a pas rempli le motif du rendez-vous", 5000, 'info')
                else
                    RageUI.CloseAll()
                    TriggerServerEvent("Rdv:gendarmerieMotif", nomprenom, numero, heurerdv, rdvmotif)
                    -- ESX.ShowNotification("~b~gendarmerie~s~\nDemande de Rendez-vous envoyée")
                    exports['okokNotify']:Alert("Gendarmerie Nationale", "Demande de Rendez-vous envoyée", 5000, 'info')
                    nomprenom = nil
                    numero = nil
                    heurerdv = nil
                    rdvmotif = nil
...
local npc = {
	{hash="s_m_m_prisguard_01",x = 1862.3557128906, y = 3691.4111328125, z = 34.327262878418, a = 159.08563232421875},
}
